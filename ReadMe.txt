================================================================================
    MFC 라이브러리 : APK 프로젝트 개요
===============================================================================

응용 프로그램 마법사에서 이 APK 응용 프로그램을 만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 프로그램을 작성하기 위한 기본 구조를 제공합니다.

APK 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.
...
APK.h
    ...
    여기에는 resource.h를 비롯한 다른 프로젝트별 헤더가 포함되어 있으며 CAPKApp 응용 프로그램 클래스가 선언되어 있습니다.

APK.cpp
    이는 응용 프로그램 클래스 CAPKApp가 포함된 기본 응용 프로그램 소스 파일입니다.
...
res\APK.ico
    아이콘 파일이며, 응용 프로그램의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 APK.rc에 의해 포함됩니다.
...
res\Toolbar.bmp
    ...
    초기 도구 모음과 상태 표시줄은 CMainFrame 클래스에서 생성됩니다. 이 도구 모음 비트맵을 리소스 편집기로 편집하고, APK.rc의 IDR_MAINFRAME TOOLBAR 배열을 업데이트하여 도구 모음 단추를 추가합니다.
/////////////////////////////////////////////////////////////////////////////

응용 프로그램 마법사에서 문서 형식과 뷰를 한 개씩 만듭니다.

APKDoc.h, APKDoc.cpp - 문서
    이 파일에는 CAPKDoc 클래스가 포함됩니다. 이 파일을 편집하여 특수 문서 데이터를 추가하고 CAPKDoc::Serialize를 통해 파일 저장 및 로드를 구현합니다.
    문서에는 다음과 같은 문자열이 있습니다.
        파일 확장명:      APK
        파일 형식 ID:        APK.Document
        주 프레임 캡션:  APK
        Doc 형식 이름:       APK
        필터 이름:         APK Files (*.APK)
        파일의 새 약식 이름: APK
        파일 형식의 긴 이름: APK.Document

APKView.h, APKView.cpp - 문서 뷰
    이 파일에는 CAPKView 클래스가 포함됩니다.
    CAPKView 개체는 CAPKDoc 개체를 보는 데 사용됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일 APK.pch와 미리 컴파일된 형식(PCT) 파일 StdAfx.obj를 빌드하는 데 사용됩니다.
