...
IMPLEMENT_DYNAMIC(TeachOCRFontTray, CDialogEx)

using namespace VisionProcess;
using namespace Euresys::Open_eVision_2_11;

TeachOCRFontTray::TeachOCRFontTray(CWnd* pParent /*=NULL*/)
	: CDialogEx(TeachOCRFontTray::IDD, pParent)
{
	m_SegmentFileData.clear();
	m_OCRFontLoadData.clear();
	m_OCRFontReadingData.clear();

	m_strFontPatch = CModelInfo::Instance()->GetRecipeRootPath() + _T("_Common_Recipe\\TrayInputFont.OCR");
...
TeachOCRFontTray::~TeachOCRFontTray()
{
	CString strTeachingOCRPatch;
	strTeachingOCRPatch = m_strFontPatch;
	m_ImageObjectForOcr.Destroy();
}

void TeachOCRFontTray::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(TeachOCRFontTray, CDialogEx)
	ON_WM_CREATE()
	ON_WM_PAINT()
	ON_BN_CLICKED(IDOK, &TeachOCRFontTray::OnBnClickedOk)
	ON_WM_CTLCOLOR()

	ON_BN_CLICKED(IDC_FONTTEACH_BTN_FONTSAVE,			&TeachOCRFontTray::OnBnClickedFontteachBtnFontsave)
	ON_BN_CLICKED(IDC_FONTTEACH_BTN_VIEWREFRESH,		&TeachOCRFontTray::OnBnClickedFontteachBtnViewrefresh)
	ON_BN_CLICKED(IDC_FONTTEACH_BTN_REREGISTER_FONT,	&TeachOCRFontTray::OnBnClickedFontteachBtnReregisterFont)

	ON_WM_LBUTTONDOWN()
END_MESSAGE_MAP()

BOOL TeachOCRFontTray::OnInitDialog()
{
...
int TeachOCRFontTray::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_ImageViewForOcr.SetTitle(_T("View : Tray Tab OCR Font"), FALSE);
...
HBRUSH TeachOCRFontTray::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void APIENTRY TeachOCRFontTray::ImageViewDrawExt_OCR(IxDeviceContext* pIDC, CDC* pDC, UINT nIndexData, LPVOID lpUsrData)
{
	TeachOCRFontTray* pOCRFontDlg = (TeachOCRFontTray*)lpUsrData;
	return;
}

BOOL APIENTRY TeachOCRFontTray::ImageViewEvent_OCR(ImageViewEvent::Event evt, UINT nIndexData, LPVOID lpUsrData)
{
...
BOOL APIENTRY TeachOCRFontTray::ImageViewMouseEvent_OCR(DWORD dwMsg, CPoint& point, UINT nIndexData, LPVOID lpUsrData)
{
	TeachOCRFontTray* pOCRFontDlg = (TeachOCRFontTray*)lpUsrData;
...
void TeachOCRFontTray::AddFontData(int nSelectIndex, CPoint ptSelectPos)
{
...
void TeachOCRFontTray::OCRSegmentReading(BOOL bSave /*=FALSE*/)
{
	USES_CONVERSION;
	...
	pGO.Reset();
	if (!CVisionSystem::Instance()->OCRMeasureInspection(&pGO, &m_ImageObjectForOcr, m_TrayData, bSave))
	{
	...
	// ----- Read Data Setting -----
	m_OCRFontReadingData.clear();
	m_OCRFontReadingData.vstrReadingText = m_TrayData.strSegmentText;
	m_OCRFontReadingData.vreReadingArea = m_TrayData.reSegmentArea;
	for (int nIndex = 0; nIndex < (int)m_TrayData.reSegmentArea.size(); nIndex++)
	{
		m_OCRFontReadingData.vbTeaching.push_back(FALSE);
		m_OCRFontReadingData.vstrTeachingText.push_back(_T(""));
	}

	if(!bSave)		m_ImageViewForOcr.ImageUpdate();
}

void TeachOCRFontTray::OCRSegmentBtnCheck()
{
	USES_CONVERSION;
...
void TeachOCRFontTray::ViewGraphicObjectUpdate()
{
...
void TeachOCRFontTray::OnBnClickedFontteachBtnFontsave()
{
	USES_CONVERSION;
	...
	strFontFileA = CW2A(m_strFontPatch);

	EOCR OCRFont;
	OCRFont.Load(T2A(m_strFontPatch)); // strFontFileA.GetBuffer(strFontFileA.GetLength()));
	...
	strFontCopyPatch += _T("\\Backup_TrayOCRFont_") + strTime + _T(".OCR");
...
void TeachOCRFontTray::OnBnClickedFontteachBtnViewrefresh()
{
	OCRSegmentReading();
	m_ImageViewForOcr.ImageUpdate();
}

void TeachOCRFontTray::OnBnClickedFontteachBtnReregisterFont()
{
	TeachOCRReRegistration ReRegistrationDlg;
	ReRegistrationDlg.SetOcrMode(OCR_TRAY);
	
	if (ReRegistrationDlg.DoModal() != IDOK) return;
}

void TeachOCRFontTray::OnBnClickedOk()
{
	CDialogEx::OnOK();
}
...
void TeachOCRFontTray::OnLButtonDown(UINT nFlags, CPoint point)
{
