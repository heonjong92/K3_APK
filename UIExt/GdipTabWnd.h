...
extern const UINT WM_GDIP_TABWND_SELCHANGE;
extern const UINT WM_GDIP_TABWND_SELCHANGED;

// CGdipTabWnd

class CGdipTabWnd : public CGdipStatic
{
	DECLARE_DYNAMIC(CGdipTabWnd)
...
public:
	...
	void RemoveTab( INT_PTR nI );
	void ResetTab();

	void SelectTab( INT_PTR nI, BOOL bNotify=TRUE );

	void EnableTab( INT_PTR nI, BOOL bEnable=TRUE );
	void ShowTab( INT_PTR nI, BOOL bShow=TRUE );
	void EnableAllTabs();
	void ShowAllTabs();

	INT_PTR GetTabCount() const { return m_aryTabInfo.GetCount(); }

	INT_PTR GetActiveTab() const { return m_nActiveTabIdx; }
...
protected:
	...
	CArray<INT_PTR>		m_aryHideTabIdx;
	CArray<INT_PTR>		m_aryDisabledTabIdx;
...
protected:
	...
	void DrawTab( Graphics& g, Rect& rectTab, const TabInfo& ti, BOOL bHover, BOOL bActive, BOOL bNotify, BOOL bDisabled );

	CSize CalcTabSize( Graphics& g, const TabInfo& ti );
	void CalcChildArea( CRect rc );

	INT_PTR IsNotifyTab( INT_PTR nI );
	INT_PTR IsDisabledTab( INT_PTR nI );
	INT_PTR IsHideTab( INT_PTR nI );
...
inline BOOL CGdipTabWnd::IsHorzTab()
{
	return (m_eTabDirection == CGdipTabWnd::TabDirectionHorzLeftTop || m_eTabDirection == CGdipTabWnd::TabDirectionHorzRightTop) ? TRUE : FALSE;
}
