...
void CVisionSystem::Initialize( HWND hWndUI )
{
	...
	// Maria DB Open
	CSplashWnd::ShowMessage(CSplashWnd::MessageTypeNormal, _T("Maria DB Open Start..."));
	MariaDBOpen();

//	std::vector<SBubblewrapExportRow> vec;
//	if (!LoadBubblewrapExport(m_MariaDB, vec))
//	{
//		AfxMessageBox(m_MariaDB.GetLastErrorMsg());
//		return;
//	}
//
//	// 확인용 출력(예: 로그/디버그)
...
//	{
//		CString s;
//		s.Format(_T("[%zu] sisd=%s, W=%.3f, L=%.3f, desc=%s"),
//			i,
//			vec[i].bubble_wrap_sisd,
//			vec[i].width_mm,
//			vec[i].length_mm,
//			vec[i].desciption);
//
//		// WRITE_LOG(WL_DEBUG, s);
//		OutputDebugString(s + _T("\r\n"));
//	}
...
//}
//
//BOOL CVisionSystem::LoadBubblewrapExport(CMariaDbClient& db, std::vector<SBubblewrapExportRow>& outRows)
//{
//	outRows.clear();
//
//	// 필요한 컬럼만 조회
//	CString sql =
//		_T("SELECT bubble_wrap_sid, width_mm, length_mm, description ")
//		_T("FROM bubblewrap_export");
//
//	std::vector<CString> cols;
//	std::vector< std::vector<CString> > rows;
//
//	if (!db.Query(sql, cols, rows))
//		return FALSE;
//
...
//
//		SBubblewrapExportRow item;
//		item.bubble_wrap_sisd = rr[0];
//		item.width_mm = ToDoubleSafe(rr[1]);
//		item.length_mm = ToDoubleSafe(rr[2]);
//		item.desciption = rr[3];
//
//		outRows.push_back(item);
//	}
//
//	return TRUE;
//}
...
BOOL CVisionSystem::CreateImageGrabBuffers(eCamType nCamtype, int nWidth, int nHeight, int nViewIndex)
{
	...
	case CamTypeAreaScan:
		...
		CxImageObject::ChannelSeqModel echannelseqmodel = CxImageObject::ChannelSeqGray;
		...
		if (m_ImageObjectArea[nViewIndex].GetWidth() == nWidth && m_ImageObjectArea[nViewIndex].GetHeight() == nHeight)
		{
			CxImageObject::ChannelSeqModel eCurrentSeq = m_ImageObjectArea[nViewIndex].GetChannelSeq();
			BOOL bChannelSeqMatched = (echannelseqmodel == eCurrentSeq) ? TRUE : FALSE;
...
void CVisionSystem::MariaDBOpen()
{
	if (!m_MariaDB.Open(_T("192.168.11.99"), _T("Vision"), _T(""), _T("k3_autopacking")))
	{
		CString msg;
		msg.Format(_T("Maria DB Open... Failure (%u) %s"), m_MariaDB.GetLastErrorCode(), m_MariaDB.GetLastErrorMsg());

		WriteMessage(LogTypeError, msg);
		CSplashWnd::ShowMessage(CSplashWnd::MessageTypeNormal, msg);
	}
	else
	{
		WriteMessage(LogTypeNotification, _T("Maria DB Open... OK"));
		CSplashWnd::ShowMessage(CSplashWnd::MessageTypeNormal, _T("Maria DB Open... OK"));
	}
...
void CVisionSystem::DEE201S(const CString& strReceiveBuf, const CString& strVisionCode)
{
	...
	strSendBuf = strVisionCode + strCommandCode + _T("SIZE|") + strResultData + m_strTraySID + _T("|") + m_strTrayOcrData + _T("|^");
